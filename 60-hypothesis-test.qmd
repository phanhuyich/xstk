...
$$
với
$\Delta_K\is\left\{\p\in\R_+^K: \sum_{i=1}^K p_i=1\right\}.$
Khi đó
$\sqrt{n}(\hat{\p}-\p^*)$
tiến về phân phối chuẩn (@def-asymp-normal),
còn
$$
n\sum_{i=1}^K\frac{(\hat{p}_i-p^*_i)^2}{p^*_i}
\todist
\chi_{K-1}^2
$$
tiến về phân phối $\chi^2$
(@thm-mle-asym)
với độ tự do giảm mất $1$ từ $K,$ vì có ràng buộc
$$
\sum_{i=1}^K p_i = 1
.$$
:::
